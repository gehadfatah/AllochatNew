...
public class AddUserInteractor implements AddUserContract.Interactor {
    ...
    @Override
    public void addUserToDatabase(final Context context,String phone, FirebaseUser firebaseUser) {
        DatabaseReference database = FirebaseDatabase.getInstance().getReference();
        User user = new User(firebaseUser.getUid(),
                firebaseUser.getEmail(),
                new SharedPrefUtil(context).getString(Constants.ARG_FIREBASE_TOKEN),
        phone,"true",new SharedPrefUtil(context).getString(Constants.ARG_USER_password));
